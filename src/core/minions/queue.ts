      ...
      if (typeof submittedModel === 'string' && submittedModel.length > 0) {
        ...
        if (verdict === 'unusable:no_tools') {
          throw new Error(
            ...
            `Pick a provider that supports tools (anthropic, openai, google, openrouter, opencode, litellm-proxy, deepseek, groq, together, azure-openai).`,
          );
        }
        if (verdict === 'unknown') {
          throw new Error(
            ...
            `Known providers: anthropic, openai, google, openrouter, opencode, litellm-proxy, ollama, llama-server, ` +
